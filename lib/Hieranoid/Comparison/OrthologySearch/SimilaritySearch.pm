...
sub BUILD {
	...
	my ( $leftDaughter, $rightDaughter ) = (
		$self->nodeObject->leftDaughter->name,
		$self->nodeObject->rightDaughter->name
	);
	if (   $self->configuration->use_outgroup
		&& $self->nodeObject->outgroupDaughter )
	{
		...
		$self->resultsObject(
			ResultsSimilaritySearch->new(
				simAA => $self->nodeObject->fileInformation->outputDirectory
				  . "/"
				  . $leftDaughter . "-"
				  . $leftDaughter,
				simAB => $self->nodeObject->fileInformation->outputDirectory
				  . "/"
				  . $leftDaughter . "-"
				  . $rightDaughter,
				simBA => $self->nodeObject->fileInformation->outputDirectory
				  . "/"
				  . $rightDaughter . "-"
				  . $leftDaughter,
				simBB => $self->nodeObject->fileInformation->outputDirectory
				  . "/"
				  . $rightDaughter . "-"
				  . $rightDaughter,
				simAC => $self->nodeObject->fileInformation->outputDirectory
				  . "/"
				  . $leftDaughter . "-"
				  . $outgroup,
				simBC => $self->nodeObject->fileInformation->outputDirectory
				  . "/"
				  . $rightDaughter . "-"
				  . $outgroup,
			)
...
sub start {
	...
	if ( $self->configuration->available_nodes > 1
		&& !defined( $self->configuration->jobNumber ) )
	{
		$isMasterProcess = 1;
	}
...
#exit;
	...
	my $directoryWithResultsFiles =
	  $self->nodeObject->fileInformation->outputDirectory;
  my $similaritySearchOutput;
  ...
  Comparison:
	foreach my $currentComparison (@comparison_order) {
		...
		$similaritySearchOutput =
		  $comparisons_hash{$currentComparison}{outputFile};
		$fileWithPrefilterHits = $similaritySearchOutput . "_prefilter.tab";
		my ( $leftDaughter, $rightDaughter ) = (
			$comparisons_hash{$currentComparison}{leftDaughter},
			$comparisons_hash{$currentComparison}{rightDaughter}
		);
		my ( $leftDaughterType, $rightDaughterType ) = (
			$comparisons_hash{$currentComparison}{leftDaughterType},
		...
		if ( !-e $similaritySearchOutput && !-s $similaritySearchOutput ) {
			print "\t current Comparison $currentComparison ("
			  . basename($similaritySearchOutput) . ")\n";
	   ...
	   #print "\tParallel mode, job number $jobNumber. filtered Blast search\n";
				INFO(
"\tParallel mode, job number $jobNumber. filtered Blast search"
				);
				if (
					!$self->singleBlastSearch(
						$similaritySearchOutput,
						$leftDaughter,
						$rightDaughter,
 ...
 # 2. pass
			if ( !$isMasterProcess ) {
				...
				if ( $self->nodeObject->childrenType eq "leaf_leaf" ) {
					...
					#exit;
					DEBUG(
"\tStarting 2.pass and saving in $similaritySearchOutput"
					);
					$self->do_ublastxml_2pass(
						$similaritySearchOutput,
						$leftDaughter,
						$rightDaughter,
						$leftDaughterType,
					...
					if ( $self->configuration->summarizeInformation =~
						/consensus/ )
					{
	...
	if ($isMasterProcess) {
		...
		for ( my $host_index = 0 ; $host_index < @ranges ; $host_index++ ) {
			...
			foreach my $node ( $from .. $to ) {
				...
				if ( $self->configuration->computationMode eq 'cluster' || $self->configuration->computationMode eq 'afs') {
					my $parallelSimilaritySearch =
					    $self->configuration->perl
					  . " -I $libPath -I $rootPath"
					  . "lib/ $rootPath$baseFile -j "
					  . $node . " -n "
					  . $self->nodeObject->name
					  . " -a orthologySearch -c $rootPath" . "lib/"
				...
				push( @jobStarted_files,
					$self->nodeObject->fileInformation->outputDirectory
					  . "/process_orthologySearch_started.$node" );
				push( @jobFinished_files,
					$self->nodeObject->fileInformation->outputDirectory
					  . "/process_orthologySearch_finished.$node" );
			}
...
# check used time
			...
			my $findFiles_cmd =
"find $directoryWithResultsFiles -regex '^process_orthologySearch_started.*' -mmin \+$wallTime";

			#DEBUG("\t$findFiles_cmd\n");
			my @findOldFiles = `$findFiles_cmd`;
			if (@findOldFiles) {
				print
				  "\t There are files running longer than $wallTime minutes\n";
				print "\t\tMaster process will stop here\n";
				exit;
			}
	  ...
	  Comparison:
		foreach my $currentComparison (@comparison_order) {
			print "\tFinally checking output from  "
			  . $currentComparison . "\n";
			...
			my $temp_dir = $self->configuration->tempDir;
			system("mkdir -p $temp_dir");
			my ($ftemp, $temp_file) = tempfile( DIR => $temp_dir, SUFFIX => '_temp', UNLINK => 1);
			
			#my $temp_file =
			#  $comparisons_hash{$currentComparison}{outputFile} . "_temp";
			my $findResultFiles_cmd = "find "
			  . $comparisons_hash{$currentComparison}{outputFile} . ".job*";
			...
			# then we sort it
			my $sort_cmd = "sort -T $temp_dir $temp_file > "
			  . $comparisons_hash{$currentComparison}{outputFile};
			#print "\tsorted and in: "
			print "\tBy now, results should be in ". $comparisons_hash{$currentComparison}{outputFile} . "\n";

			#print "\tconcatenating results: $cat_cmd\n$sort_cmd";
			system($sort_cmd);

      # remove temp_file which is not needed anymore
      unlink($temp_file);
      
			# Results exist?
			if (   !-e $comparisons_hash{$currentComparison}{outputFile}
				|| !-s $comparisons_hash{$currentComparison}{outputFile} )
			{
				ERROR( "Could not find hits "
					  . $comparisons_hash{$currentComparison}{leftDaughter}
					  ->name . " - "
...
sub singleBlastSearch {
	#### PARAMETER VARIABLES
	my ( $self, $output_file, $leftDaughter, $rightDaughter,
		$singleSimilaritySearchResults,
		$allowedQueryDatabaseEntities, $jobNumber )
	  = (@_);
	...
	my ( $queryName, $databaseName ) =
	  ( basename($query), basename($database) );
	my $segmasker = $self->configuration->segmasker;

	# using tmp file to avoid heavy writing to afs
	my ( undef, $ublast_output_tmp ) =
	  tempfile( SUFFIX => '.prefilter.out', UNLINK => 0 );
	my ( undef, $query_filtered_file ) =
	  tempfile( SUFFIX => '.query_filtered.fa', UNLINK => 0 );
	...
	if ($jobNumber) {
		...
		if ( !-e $query_filtered_file || !-s $query_filtered_file ) {
			ERROR(
"ERROR. Could not read assigned part of fasta file ($query_filtered_file)\n"
			);
			return
			  0
			  ; #die "ERROR. Could not read assigned part of fasta file ($query_filtered_file)\n";
		}
		$query = $query_filtered_file;
	}
	my $addAllowedQueryDatabaseEntities = 1;
	$addAllowedQueryDatabaseEntities = 0
	  if ( keys(%$allowedQueryDatabaseEntities) );
	...
	DEBUG(  "\tsimilarity search tools: "
		  . $self->configuration->similaritySearchTool
		  . "\n" );
	if (   $self->configuration->similaritySearchTool eq 'usearch'
		|| $self->configuration->similaritySearchTool eq 'ublast' )
	{
		if ( !-e $ublast_output || !-s $ublast_output ) {
			DEBUG("\tusing Usearch");
			my ( undef, $masked_query_database_file ) =
			  tempfile( SUFFIX => '.masked_query.db', UNLINK => 1 );
			unlink($masked_query_database_file)
			  if -e $masked_query_database_file;

			my $segmasker_call =
"$segmasker -in $query -out $masked_query_database_file -outfmt fasta";

			#print $segmasker_call."\n";
			system($segmasker_call);
			if (   !-e $masked_query_database_file
				|| !-s $masked_query_database_file )
			{
				WARN( "Could not mask low complexity regions in query database "
					  . basename($query)
					  . " empty $masked_query_database_file (call: $segmasker_call)\n"
				);
...
#return 0;
			...
			my $blast_call =
			    $self->configuration->usearch
			  . " --quiet --query $query  --db $database  --userout $ublast_output_tmp --userfields query+target+bits+ql+tl+qlo+qhi+tlo+thi+qs+ts "
			  . $self->configuration->ublastParameters;
...
#my $blast_call = $self->configuration->usearch."  --query $query  --db $database  --userout $ublast_output_tmp --userfields query+target+bits+ql+tl+qlo+qhi+tlo+thi+qs+ts ".$self->configuration->ublastParameters;
			...
			if ( !-e $ublast_output_tmp || !-s $ublast_output_tmp ) {
				print
"\tPrefiltering blast resulted in no hits (file: $ublast_output_tmp)\n$blast_call\n";
				ERROR(
"\tPrefiltering blast resulted in no hits (file: $ublast_output_tmp)\n$blast_call\n"
				);
			...
			my $blast2xml_call =
			    $self->configuration->perl
			  ." ".$self->configuration->blast2xml." -i $ublast_output_tmp | "
			  . $self->configuration->perl
			  . " ./blast_parser.pl "
			  . $self->configuration->similaritySearchCutoff
			  . " > $ublast_output";
	 ...
	 #print "\tPrefiltering blast resulted in no hits (file: $ublast_output)\n";
				ERROR(
"\tPrefiltering blast resulted in no hits (file: $ublast_output)\n"
				);
				return 0;
			}
		}
		...
		foreach my $aLine (@blast_parser_lines) {
			...
			if ( exists( $singleSimilaritySearchResults->{ $words[0] } ) ) {
				$singleSimilaritySearchResults->{ $words[0] } =
				    $singleSimilaritySearchResults->{ $words[0] } . " "
				  . $words[1];
			}
			else {
		...
		system( $self->configuration->formatdb . " -i $database" )
		  if !-e "$database.pin";
		sleep(10);    # Wait for formatdb
		...
		system("date");
		open FHR,
		    $self->configuration->blast
		  . " -C3 -F\"m S\" -i $query -d $database -p blastp -v $noDBSequences -b $noDBSequences -M BLOSUM62 -z 5000000 -m7 | "
		  . $self->configuration->perl . " "
		  . $self->configuration->blastParser . " 40|";
		while (<FHR>) {
			my $aLine = $_;
			chomp($aLine);
			my @words = split( /\s+/, $aLine );
	...
	my $debugString =
	    "1.Pass\t"
	  . $self->configuration->similaritySearchTool
	  . "\t$queryName ($noQuerySequences) - $databaseName ($noDBSequences):  "
	  . timestr( $time_diff, 'all' );
	attach_to_file(
		{
			file_name => $self->configuration->timeFile,
			text      => $debugString . "\n"
		}
	);
	...
	if ($jobNumber) {
		print
"\tfinished job $jobNumber. Output files should be in $ublast_output\n";
	}

	# Copy tmp results to regular file
	# Mainly for debugging reasons
	copy( $ublast_output_tmp, $ublast_output )
	  || die "\tCould not copy $ublast_output_tmp to $ublast_output";
	if ( !keys(%$singleSimilaritySearchResults) ) {
		ERROR(
"Could not parse hits from first Blast run (cat $ublastxml_output | blastParser "
			  . $self->configuration->similaritySearchCutoff
			  . ")\n" );
...
sub getHits4Process {
	...
	my $addAllowedQueryDatabaseEntities =
	  $arg_ref->{addAllowedQueryDatabaseEntities};

	my $allowedQueryDatabaseEntities = $arg_ref->{allowedQueryDatabaseEntities};
	if ( !-e $hitsFile || !-s $hitsFile ) {
		ERROR(
"Required prefilter file $hitsFile for process $ownProcessId not found"
		);
		return 0;
	}
	my %hits4query;
