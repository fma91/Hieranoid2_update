        ...
        # foreach my $og(sort {$a<=>$b} keys(%{$og_hashref})){
                ...
                foreach my $og(keys(%groupPredictionHash)){
                        ...
                        # Alignment
                                ...
                                my $tempDir = $self->configuration->tempDir;
                                if(!doAlignment({alignmentCall => $alignment_call, alignment_tmp_file => $alignment_tmp_file, current_sequences => $current_sequences, tempDir => $tempDir})){
                                        next OG;
                                }
                        # Consensus        
