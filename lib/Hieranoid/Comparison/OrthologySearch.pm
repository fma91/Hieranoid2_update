...
sub start
  {
	...
	if ( (-e $self->similaritySearch->resultsObject->simAA
		 ...
		 && -s $self->similaritySearch->resultsObject->simBB) || -e $self->nodeObject->orthologGroups->orthologyPredictionFile)
	  {
	...
	if (    $self->nodeObject->outgroupDaughter
		 ...
		 && -s $self->similaritySearch->resultsObject->simBC ) || -e $self->nodeObject->orthologGroups->orthologyPredictionFile)
	  {
		$allRequiredFilesExist = 1;
	  }
	if ( !$allRequiredFilesExist )
	...
	else
	  {
		...
		#unlink($self->nodeObject->orthologGroups->orthologyPredictionFile); # Fixes the problem of duplicated branch names in sqlTable after re-run
	  }
